...
- **ログイン画面**
  - **UI 表示**
    - [ ] アプリ起動後、未ログイン状態の場合にログイン画面が正しく表示されること。
    - [ ] アプリのタイトルと説明文が表示されること。
    - [ ] 「Google で続ける」ボタンが表示されること。
    - [ ] **iOS の場合**：「Apple で続ける」ボタンが表示されること。
    - [ ] **Android の場合**：「Apple で続ける」ボタンが表示されないこと。
    - [ ] 「アカウントを利用せず続ける」ボタンが表示されること。
    - [ ] 「利用規約」と「プライバシーポリシー」へのリンクが表示されること。
  - **Google ログイン機能**
    - [ ] **正常系**: 「Google で続ける」ボタンをタップすると、Google アカウントの選択画面が表示され、認証に成功するとホーム画面に遷移すること。
    - [ ] **正常系**: ログイン処理中に、ボタン上にローディングインジケーターが表示され、ボタンが非活性になること。
...
- **アカウント連携**
  ...
  - [ ] 連携済みの Google/Apple アカウントで連携しようとした場合、適切なエラーメッセージが表示される。
- **ログアウト・アカウント削除**
  - [ ] 設定画面からログアウトできる。
  - [ ] ログアウト後、ログイン画面に遷移する。
  - [ ] 設定画面からアカウントを削除できる。
  - [ ] アカウント削除後、ログイン画面に遷移し、削除したアカウントではログインできない。

### 3.2. ホーム画面

- **家事一覧タブ**
  - [ ] 登録済みの家事が `createdAt` の降順で表示される。
  - [ ] データ取得中にスケルトンが表示される。
  - [ ] データ取得に失敗した場合、エラーメッセージと再試行ボタンが表示される。
  - [ ] 家事が一件も登録されていない場合、「登録されている家事はありません」というメッセージが表示される。
  - [ ] 画面をプルダウンすることで、一覧が更新される。
  - [ ] 家事アイテムの左側をタップすると、家事完了ログが記録される。
  - [ ] 家事完了時、完了アイコンのアニメーションが表示される。
  - [ ] 家事完了時、「ログ」タブが一瞬ハイライトされる。
  - [ ] 家事完了時、「家事を記録しました」というスナックバーが表示される。
  - [ ] 家事アイテムの右側に、家事ダッシュボードへの遷移ボタンが**ない**こと。
  - [ ] 家事アイテムを左にスワイプすると、削除確認ダイアログが表示される。
